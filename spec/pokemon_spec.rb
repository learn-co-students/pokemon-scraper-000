...
describe "Pokemon" do
  ...
  describe "has caught all 151 from scraping" do
	it "has all 151 pokemon" do
      expect(@db.execute("SELECT COUNT(*) FROM pokemons").flatten.first).to eq(151)
    end
    ...
    it "knows Psyduck is the 54th pokemon" do
      expect(@db.execute("SELECT * FROM pokemons WHERE name = 'Psyduck'").flatten.first).to eq(54)
    end
  ...
  describe "saving more pokemon to the database" do
    ...
    it "knows the pokemon count increases" do
      expect(@db.execute("SELECT COUNT(*) FROM pokemons").flatten.first).to eq(152)
    end

    it "has Togepi as the last pokemon" do
      expect(@db.execute("SELECT name FROM pokemons ORDER BY id DESC").flatten.first).to eq("Togepi")
    end
  end
  ...
  describe "BONUS" do
    ...
    it "alters Pikachu's hp to 59" do
      pikachu.alter_hp(59)
      expect(@db.execute("SELECT hp FROM pokemons WHERE id = 25").flatten.first).to eq(59)
    end

    # Now we alter Magikarp's hp
    it "alters Magikarp's hp" do
      magikarp.alter_hp(0)
      expect(@db.execute("SELECT hp FROM pokemons WHERE id = 129").flatten.first).to eq(0)
    end
