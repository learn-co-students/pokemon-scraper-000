...
describe "Pokemon" do
  ...
  describe "has caught all 151 from scraping" do
    it "has all 151 pokemon" do
      expect(@db.execute("SELECT COUNT(*) FROM pokemon_table").flatten.first).to eq(151)
    end

    it "knows all the information about Horsea" do
      expect(@db.execute("SELECT * FROM pokemon_table WHERE name = 'Horsea'").flatten).to eq([116, "Horsea", "Water"])
    end

    it "knows Psyduck is the 54th pokemon" do
      expect(@db.execute("SELECT id FROM pokemon_table WHERE name = 'Psyduck'").flatten.first).to eq(54)
    end

    it "knows pokemon with the id 143 is Snorlax" do
      expect(@db.execute("SELECT name FROM pokemon_table WHERE id = '143'").flatten.first).to eq("Snorlax")
    end

    it "knows Charmander's type is fire" do
      expect(@db.execute("SELECT type FROM pokemon_table WHERE name = 'Charmander'").flatten.first).to eq("Fire")
    end
  end
  ...
  describe "saving more pokemon to the database" do
    ...
    it "knows the pokemon count increases" do
      expect(@db.execute("SELECT COUNT(*) FROM pokemon_table").flatten.first).to eq(152)
    end

    it "has Togepi as the last pokemon" do
      expect(@db.execute("SELECT name FROM pokemon_table WHERE id = '152'").flatten.first).to eq("Togepi")
    end
  end
  ...
  describe "BONUS" do
    ...
    it "knows that a pokemon have a default hp of 60" do
      expect(@db.execute("SELECT hp FROM pokemon_table LIMIT 1").flatten.first).to eq(60)
    end
    ...
    it "alters Pikachu's hp to 59" do
      pikachu.alter_hp(59)
      expect(@db.execute("SELECT hp FROM pokemon_table WHERE name = 'Pikachu'").flatten.first).to eq(59)
    end

    # Now we alter Magikarp's hp
    it "alters Magikarp's hp" do
      magikarp.alter_hp(0)
      expect(@db.execute("SELECT hp FROM pokemon_table WHERE name = 'Magikarp'").flatten.first).to eq(0)
    end
